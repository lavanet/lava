...
async function main() {
    ...
    for (let i = 0; i < 80; i++) { // send relays synchronously
        try {
            ...
            // Validate chainID
            if (chainID !== "lava") {
                throw new Error(" ERR [tendermintrpc_chainid_fetch_badge_err] Chain ID is not equal to lava");
            } else {
                console.log(i, "[tendermintrpc_chainid_fetch_badge_err] Success: Fetching Lava chain ID using tendermintrpc passed. Chain ID correctly matches 'lava'");
            }
        } catch (error) {
            throw new Error(` ERR ${i} [tendermintrpc_chainid_fetch_badge_err] failed sending relay tendermint test: ${error.message}`);
        }
    }
}
...
(async () => {
    try {
        ...
        console.error(" ERR [tendermintrpc_chainid_fetch_badge_err] " + error.message);
        process.exit(1);
    }
})();