...
var whitelist = map[string]string{
	...
	`error connecting to provider error="context deadline exceeded"`: "This error is also caused by the initial bootup, continuous failure would be catched by the e2e so we can whitelist this error.",
	"purging provider after all endpoints are disabled provider":     "This error is also caused by the initial bootup, continuous failure would be catched by the e2e so we can whitelist this error.",
