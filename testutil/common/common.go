...
func CreateMsgDetectionTest(ctx context.Context, consumer Account, provider0 Account, provider1 Account, spec spectypes.Spec) (detectionMsg *conflicttypes.MsgDetection, reply1 *types.RelayReply, reply2 *types.RelayReply, errRet error) {
	...
	sig, err := sigs.SignStruct(consumer.SK, *msg.ResponseConflict.ConflictRelayData0.Request.RelaySession, sigs.PrepareRelaySessionForSignature)
	if err != nil {
	...
	sig, err = sigs.SignStruct(consumer.SK, *msg.ResponseConflict.ConflictRelayData1.Request.RelaySession, sigs.PrepareRelaySessionForSignature)
	if err != nil {
		return msg, nil, nil, err
	}
