...
func TestAdaptiveMaxCalculator_NilHandling(t *testing.T) {
	...
	require.True(t, ok, "stats[enabled] should be bool")
...
func TestAdaptiveMaxCalculator_Stats(t *testing.T) {
	...
	adaptiveMaxP95, ok := stats["adaptive_max_p95"].(float64)
	require.True(t, ok, "stats[adaptive_max_p95] should be float64")
	require.Greater(t, adaptiveMaxP95, 0.0)

	adaptiveP10, ok := stats["adaptive_p10"].(float64)
	require.True(t, ok, "stats[adaptive_p10] should be float64")
	require.Greater(t, adaptiveP10, 0.0)

	adaptiveP90, ok := stats["adaptive_p90"].(float64)
	require.True(t, ok, "stats[adaptive_p90] should be float64")
	require.Greater(t, adaptiveP90, 0.0)

	median, ok := stats["median"].(float64)
	require.True(t, ok, "stats[median] should be float64")
	require.Greater(t, median, 0.0)

	p99, ok := stats["p99"].(float64)
	require.True(t, ok, "stats[p99] should be float64")
	require.Greater(t, p99, 0.0)

	totalWeight, ok := stats["total_weight"].(uint64) // Changed from float64 to uint64
	require.True(t, ok, "stats[total_weight] should be uint64")
	require.Greater(t, totalWeight, uint64(0))

	centroidCount, ok := stats["centroid_count"].(int)
	require.True(t, ok, "stats[centroid_count] should be int")
	require.Greater(t, centroidCount, 0)

	compression, ok := stats["compression"].(float64)
	require.True(t, ok, "stats[compression] should be float64")
	require.Equal(t, 100.0, compression)

	halfLifeSeconds, ok := stats["half_life_seconds"].(float64)
	require.True(t, ok, "stats[half_life_seconds] should be float64")
	require.Equal(t, 3600.0, halfLifeSeconds)

	minMax, ok := stats["min_max"].(float64)
	require.True(t, ok, "stats[min_max] should be float64")
	require.Equal(t, 1.0, minMax)

	maxMax, ok := stats["max_max"].(float64)
	require.True(t, ok, "stats[max_max] should be float64")
	require.Equal(t, 30.0, maxMax)
}
...
func TestAdaptiveMaxCalculator_LargeDataset(t *testing.T) {
	...
	// Verify stats
	stats := calc.GetStats()
	totalWeight, ok := stats["total_weight"].(uint64)
	require.True(t, ok, "stats[total_weight] should be uint64")
	require.Greater(t, totalWeight, uint64(800)) // Should have ~900 total weight
}
...
func TestAdaptiveMaxCalculator_Stats_IncludesBounds(t *testing.T) {
	...
	p10, ok := stats["adaptive_p10"].(float64)
	require.True(t, ok, "stats[adaptive_p10] should be float64")
	p90, ok := stats["adaptive_p90"].(float64)
	require.True(t, ok, "stats[adaptive_p90] should be float64")
	p95, ok := stats["adaptive_max_p95"].(float64)
	require.True(t, ok, "stats[adaptive_max_p95] should be float64")
