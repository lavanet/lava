...
Servicer | ERR_stake_servicer_new_amount | Tx | sent when insufficient amount to pay for requested stake | servicer | staked servicer address | deadline | the block height in which the servicer can start getting pairings | stake | the stake deposited by the servicer | requestedDeadline | the deadline the servicer requested | error | the error that was sent
