...
func (ts *tester) setupForIprpcTests(fundIprpcPool bool) {
	...
	err = ts.StakeProvider(provider, pAcc.GetVaultAddr(), ts.specs[0], testStake)
	require.NoError(ts.T, err)
	err = ts.StakeProvider(provider, pAcc.GetVaultAddr(), ts.specs[1], testStake)
	require.NoError(ts.T, err)
	err = ts.StakeProvider(provider2, p2Acc.GetVaultAddr(), ts.specs[0], testStake)
	require.NoError(ts.T, err)
	err = ts.StakeProvider(provider2, p2Acc.GetVaultAddr(), ts.specs[1], testStake)
	require.NoError(ts.T, err)
