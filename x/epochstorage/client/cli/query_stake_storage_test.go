...
func networkWithStakeStorageObjects(t *testing.T, n int) (*network.Network, []types.StakeStorage) {
	...
	for i := 0; i < n; i++ {
		...
		nullify.Fill(&stakeStorage)
		state.StakeStorageList = append(state.StakeStorageList, stakeStorage)
	}
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), state.StakeStorageList
}

func TestShowStakeStorage(t *testing.T) {
	net, objs := networkWithStakeStorageObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowStakeStorage(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetStakeStorageResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.StakeStorage)
				require.Equal(t,
					nullify.Fill(&tc.obj),
...
func TestListStakeStorage(t *testing.T) {
	net, objs := networkWithStakeStorageObjects(t, 5)
