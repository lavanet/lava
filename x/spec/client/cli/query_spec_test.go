...
func networkWithSpecObjects(t *testing.T, n int) (*network.Network, []types.Spec) {
	...
	return network.New(t, cfg), state.SpecList
}

func TestShowSpec(t *testing.T) {
	net, objs := networkWithSpecObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetSpecResponse
...
func TestListSpec(t *testing.T) {
	net, objs := networkWithSpecObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListSpec(), args)
			require.NoError(t, err)
			var resp types.QueryAllSpecResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
