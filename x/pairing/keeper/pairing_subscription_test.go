...
func TestRelayPaymentSubscription(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			relayRequest.Sig, err = sigs.SignStruct(consumer.SK, *relayRequest, sigs.PrepareRelaySession)
			require.Nil(t, err)
...
func TestRelayPaymentSubscriptionCU(t *testing.T) {
	...
	for ; uint64(i) < ts.plan.PlanPolicy.GetTotalCuLimit()/ts.plan.PlanPolicy.GetEpochCuLimit(); i++ {
		...
		relayRequest.SessionId = uint64(i)
		relayRequest.Sig, err = sigs.SignStruct(consumerA.SK, *relayRequest, sigs.PrepareRelaySession)
		require.Nil(t, err)
		_, err = ts.servers.PairingServer.RelayPayment(ts.ctx, &types.MsgRelayPayment{Creator: ts.providers[0].Addr.String(), Relays: []*types.RelaySession{relayRequest}})
		require.Nil(t, err)
	...
	relayRequest.Sig, err = sigs.SignStruct(consumerB.SK, *relayRequest, sigs.PrepareRelaySession)
	require.Nil(t, err)
	_, err = ts.servers.PairingServer.RelayPayment(ts.ctx, &types.MsgRelayPayment{Creator: ts.providers[0].Addr.String(), Relays: []*types.RelaySession{relayRequest}})
	require.NotNil(t, err)
}
	...
	for _, tt := range providersToPairTestTemplates {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wasteSubscriptionCu {
				...
				relayRequest.Sig, err = sigs.SignStruct(consumerToWasteCu.SK, *relayRequest, sigs.PrepareRelaySession)
				require.Nil(t, err)
				_, err = ts.servers.PairingServer.RelayPayment(ts.ctx, &types.MsgRelayPayment{Creator: ts.providers[0].Addr.String(), Relays: []*types.RelaySession{relayRequest}})
				require.Nil(t, err)
...
func TestPairingNotChangingDueToCuOveruse(t *testing.T) {
	...
	for i := 0; i < int(ts.plan.PlanPolicy.TotalCuLimit)/int(ts.plan.PlanPolicy.EpochCuLimit); i++ {
		...
		relayRequest.SessionId = uint64(i)
		relayRequest.Sig, err = sigs.SignStruct(ts.clients[0].SK, *relayRequest, sigs.PrepareRelaySession)
		require.Nil(t, err)
