...
func TestRelayPaymentMemoryTransferAfterEpochChange(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			sig, err := sigs.SignStruct(ts.clients[0].SK, *relaySession, sigs.PrepareRelaySession)
...
func TestRelayPaymentNotUnstakingProviderForUnresponsivenessIfNoEpochInformation(t *testing.T) {
	...
	for clientIndex := 0; clientIndex < testClientAmount; clientIndex++ { // testing testClientAmount of complaints
		...
		sig, err := sigs.SignStruct(ts.clients[clientIndex].SK, *relaySession, sigs.PrepareRelaySession)
		relaySession.Sig = sig
		require.Nil(t, err)
		Relays = append(Relays, relaySession)
	}
...
func TestRelayPaymentNotUnstakingProviderForUnresponsivenessBecauseOfServices(t *testing.T) {
	...
	for i := 0; i < 2; i++ { // move to epoch 3 so we can check enough epochs in the past
		...
		sig, err := sigs.SignStruct(ts.clients[i].SK, *relaySession, sigs.PrepareRelaySession)
		relaySession.Sig = sig
		require.Nil(t, err)
...
func TestCuUsageInProjectsAndSubscription(t *testing.T) {
	...
	relaySession.Sig, err = sigs.SignStruct(proj1SK, *relaySession, sigs.PrepareRelaySession)
	require.Nil(t, err)
...
func TestBadgeValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			relaySession.Sig, err = sigs.SignStruct(tt.relaySigner.SK, *relaySession, sigs.PrepareRelaySession)
			require.Nil(t, err)
...
func TestAddressEpochBadgeMap(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		relaySession.Sig, err = sigs.SignStruct(badgeUser.SK, *relaySession, sigs.PrepareRelaySession)
		require.Nil(t, err)
