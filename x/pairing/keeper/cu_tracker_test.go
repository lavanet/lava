...
func TestAddingTrackedCu(t *testing.T) {
	...
	cu, found, _ := ts.Keepers.Subscription.GetTrackedCu(ts.Ctx, sub.Consumer, provider1Addr, ts.spec.Index, sub.Block)
	require.True(t, found)
	require.Equal(t, cuSum, cu)

	_, found, _ = ts.Keepers.Subscription.GetTrackedCu(ts.Ctx, sub.Consumer, provider2Addr, ts.spec.Index, sub.Block)
	require.False(t, found)

	// check trackedCU was created on the sub's creation block (and not before)
	_, found, _ = ts.Keepers.Subscription.GetTrackedCu(ts.Ctx, sub.Consumer, provider1Addr, ts.spec.Index, sub.Block-1)
	require.False(t, found)
}
