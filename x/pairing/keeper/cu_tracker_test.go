...
func TestAddingTrackedCu(t *testing.T) {
	...
	cu, found, _ := ts.Keepers.Subscription.GetTrackedCu(ts.Ctx, sub.Consumer, provider1Addr, sub.Block)
	require.True(t, found)
	require.Equal(t, cuSum, cu)

	_, found, _ = ts.Keepers.Subscription.GetTrackedCu(ts.Ctx, sub.Consumer, provider2Addr, sub.Block)
	require.False(t, found)

	// check trackedCU was created on the sub's creation block (and not before)
	_, found, _ = ts.Keepers.Subscription.GetTrackedCu(ts.Ctx, sub.Consumer, provider1Addr, sub.Block-1)
	require.False(t, found)
}
