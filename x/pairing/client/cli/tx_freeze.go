...
func CmdFreeze() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `The freeze command allows a provider to freeze its activity (starting from the next epoch). This 
		allows the provider to stop providing service to consumers without worrying about bad QoS. 
		The provider won't be paired with consumers when it's freezed. To unfreeze, the provider must send an unfreeze 
		transaction. Example use case: a provider wishes to halt its services due to maintenance.`,
