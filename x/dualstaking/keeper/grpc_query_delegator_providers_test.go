...
func TestQueryWithUnbonding(t *testing.T) {
	...
	delegation := types.NewDelegation(delegator, provider, spec.Index, ts.Ctx.BlockTime())
...
func TestQueryWithPendingDelegations(t *testing.T) {
	...
	delegation1 := types.NewDelegation(delegator1, provider, spec.Index, ts.Ctx.BlockTime())
	...
	delegation2 := types.NewDelegation(delegator2, provider, spec.Index, ts.Ctx.BlockTime())
...
func TestQueryProviderMultipleDelegators(t *testing.T) {
	...
	for i := 0; i < len(delegators); i++ {
		...
		delegation := types.NewDelegation(delegators[i], provider, chainID, ts.Ctx.BlockTime())
		delegation.Amount = amount
...
func TestQueryDelegatorMultipleProviders(t *testing.T) {
	...
	for i := 0; i < len(providers); i++ {
		...
		delegation := types.NewDelegation(delegator, providers[i], spec.Index, ts.Ctx.BlockTime())
		delegation.Amount = amount
		delegations = append(delegations, delegation)
	}
