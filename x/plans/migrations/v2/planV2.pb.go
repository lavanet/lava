...
func (m *PlanV2) Reset()         { *m = PlanV2{} }
func (m *PlanV2) String() string { return proto.CompactTextString(m) }
...
var xxx_messageInfo_PlanV2 proto.InternalMessageInfo
...
func init() {
	proto.RegisterType((*PlanV2)(nil), "lavanet.lava.plans.PlanV2")
}

func init() { proto.RegisterFile("plans/plan.proto", fileDescriptor_e5909a10cd0e3497) }

var fileDescriptor_e5909a10cd0e3497V2 = []byte{
...
func (this *PlanV2) Equal(that interface{}) bool {
	...
	that1, ok := that.(*PlanV2)
	if !ok {
		that2, ok := that.(PlanV2)
		if ok {
			that1 = &that2
		} else {
			return false
...
func encodeVarintPlanV2(dAtA []byte, offset int, v uint64) int {
	offset -= sovPlan(v)
	base := offset
	for v >= 1<<7 {
...
func sovPlanV2(x uint64) (n int) {
	return (math_bits.Len64(x|1) + 6) / 7
}
func sozPlanV2(x uint64) (n int) {
...
func skipPlanV2(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	depth := 0
	for iNdEx < l {
...
var (
	ErrInvalidLengthPlanV2        = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowPlanV2          = fmt.Errorf("proto: integer overflow")
	ErrUnexpectedEndOfGroupPlanV2 = fmt.Errorf("proto: unexpected end of group")
)
