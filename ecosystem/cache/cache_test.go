...
func initTest() (context.Context, *cache.RelayerCacheServer) {
	...
	cs.InitCache(ctx, cache.DefaultExpirationTimeFinalized, cache.DefaultExpirationForNonFinalized, 1, cache.DisabledFlagOption)
...
func TestCacheExpirationMultiplier(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Multiplier of 1", multiplier: 1, expected: cache.DefaultExpirationForNonFinalized},
		{name: "Multiplier of 1.2", multiplier: 1.2, expected: time.Duration(float64(cache.DefaultExpirationForNonFinalized) * 1.2)},
		{name: "Multiplier of 2", multiplier: 2, expected: time.Duration(float64(cache.DefaultExpirationForNonFinalized) * 2)},
		{name: "Multiplier of 2.5", multiplier: 2.5, expected: time.Duration(float64(cache.DefaultExpirationForNonFinalized) * 2.5)},
		{name: "Multiplier of 200", multiplier: 200, expected: time.Duration(float64(cache.DefaultExpirationForNonFinalized) * 200)},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cs := cache.CacheServer{CacheMaxCost: 2 * 1024 * 1024 * 1024}
			cs.InitCache(context.Background(), cache.DefaultExpirationTimeFinalized, cache.DefaultExpirationForNonFinalized, tt.multiplier, cache.DisabledFlagOption)
			cacheServer := &cache.RelayerCacheServer{CacheServer: &cs}
