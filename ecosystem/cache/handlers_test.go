...
func TestCompressDecompress(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// For non-empty data, compressed should be valid gzip
			if len(tt.data) > 0 {
...
func TestFormatCacheValueCompression(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			responseData:       bytes.Repeat([]byte("a"), CompressionThreshold),
			finalized:          true,
		...
		},
		{
			...
			responseData:       bytes.Repeat([]byte("a"), CompressionThreshold+1),
			finalized:          true,
			expectCompressed:   true,
			expectIsCompressed: true,
		},
...
func TestCompressionRatio(t *testing.T) {
	...
	compressed, err := compressData(jsonPayload)
	require.NoError(t, err)
