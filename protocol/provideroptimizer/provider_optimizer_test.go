...
func setupProviderOptimizer(maxProvidersCount int) *ProviderOptimizer {
	...
	return NewProviderOptimizer("", "", STRATEGY_BALANCED, averageBlockTIme, baseWorldLatency, uint(maxProvidersCount), nil, nil)
}
...
func TestProviderOptimizerAvailabilityBlockError(t *testing.T) {
	...
	time.Sleep(4 * time.Millisecond)
	selectionTier, _ := providerOptimizer.CalculateSelectionTiers(providersGen.providersAddresses, nil, requestCU, requestBlock, uint64(requestBlock))
...
func TestProviderOptimizerSyncScore(t *testing.T) {
	...
	selectionTier, _ = providerOptimizer.CalculateSelectionTiers(providersGen.providersAddresses, nil, requestCU, int64(syncBlock), syncBlock)
	tier0 = selectionTier.GetTier(0, 4, 3)
	for idx := range tier0 {
...
func TestProviderOptimizerStrategiesScoring(t *testing.T) {
	...
	selectionTier, _ = providerOptimizer.CalculateSelectionTiers(providersGen.providersAddresses, map[string]struct{}{providersGen.providersAddresses[2]: {}}, requestCU, requestBlock, uint64(requestBlock))
	tier0 = selectionTier.GetTier(0, 4, 3)
	require.Greater(t, len(tier0), 0) // shouldn't be empty
	...
	selectionTier, _ = providerOptimizer.CalculateSelectionTiers(providersGen.providersAddresses, map[string]struct{}{providersGen.providersAddresses[2]: {}}, requestCU, int64(syncBlock), syncBlock)
	tier0 = selectionTier.GetTier(0, 4, 3)
	require.Greater(t, len(tier0), 0) // shouldn't be empty
	require.Equal(t, providersGen.providersAddresses[0], tier0[0].Address)
}
