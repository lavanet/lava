...
func TestWaitForPairingBufferedChannelNoDeadlock(t *testing.T) {
	...
	server := &RPCConsumerServer{
		serverContext:          ctx,
		consumerSessionManager: createSlowInitializingSessionManager(ctx, 2*time.Second),
...
func TestWaitForPairingNonBlockingSendPattern(t *testing.T) {
	...
	server := &RPCConsumerServer{
		serverContext:          ctx,
		consumerSessionManager: createInitializedSessionManager(),
		listenEndpoint: &lavasession.RPCEndpoint{
			ChainID:      "TEST",
			ApiInterface: "rest",
		},
...
func TestWaitForPairingAggressiveCancellation(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		server := &RPCConsumerServer{
			serverContext:          ctx,
			consumerSessionManager: createSlowInitializingSessionManager(ctx, initDelay),
			listenEndpoint: &lavasession.RPCEndpoint{
				ChainID:      "TEST",
				ApiInterface: "rest",
			},
...
func TestWaitForPairingRaceWindowCoverage(t *testing.T) {
	...
	server := &RPCConsumerServer{
		serverContext:          ctx,
		consumerSessionManager: createSignalingSessionManager(ctx, signalChan, 100*time.Millisecond, 10*time.Millisecond),
		listenEndpoint: &lavasession.RPCEndpoint{
			ChainID:      "TEST",
			ApiInterface: "rest",
		},
...
func TestWaitForPairingMultipleTimeoutCycles(t *testing.T) {
	...
	server := &RPCConsumerServer{
		serverContext:          ctx,
		consumerSessionManager: createNeverInitializingSessionManager(),
		listenEndpoint: &lavasession.RPCEndpoint{
			ChainID:      "TEST",
			ApiInterface: "rest",
		},
