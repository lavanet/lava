...
// TestConsumerStateMachineCrossValidationNoRetry tests that when cross-validation feature is enabled,
...
func TestConsumerStateMachineCrossValidationNoRetry(t *testing.T) {
	t.Run("cross_validation_no_retry", func(t *testing.T) {
		...
		relayProcessor := relaycore.NewRelayProcessor(ctx, crossValidationParams, consistency, relaycoretest.RelayProcessorMetrics, relaycoretest.RelayProcessorMetrics, relaycoretest.RelayRetriesManagerInstance, NewRelayStateMachine(ctx, usedProviders, &ConsumerRelaySenderMock{retValue: nil, tickerValue: 10 * time.Millisecond}, protocolMessage, nil, false, relaycoretest.RelayProcessorMetrics), qos.NewQoSManager())
