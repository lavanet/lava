...
func TestNewRelayStateMachineSelectionType(t *testing.T) {
	...
	t.Run("no headers - Stateless selection", func(t *testing.T) {
		...
		sm, err := NewRelayStateMachine(ctx, usedProviders, &ConsumerRelaySenderMock{}, protocolMessage, nil, false)
		require.NoError(t, err)
...
// TestConsumerStateMachineRetryLimit verifies the full state machine stops retrying
// when the RelayRetryLimit is reached for both node and protocol errors.
func TestConsumerStateMachineRetryLimit(t *testing.T) {
	tests := []struct {
		name             string
		retryLimit       int
