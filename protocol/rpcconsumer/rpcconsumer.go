...
func (rpcc *RPCConsumer) Start(ctx context.Context, txFactory tx.Factory, clientCtx client.Context, rpcEndpoints []*lavasession.RPCEndpoint, requiredResponses int, cache *performance.Cache, strategy provideroptimizer.Strategy, metricsListenAddress string, maxConcurrentProviders uint, usageServerAddr string) (err error) {
	...
	consumerUsageserveManager := metrics.NewConsumerUsageserverClient(usageServerAddr) // start up prometheus metrics
...
rpcconsumer consumer_examples/full_consumer_example.yml --cache-be "127.0.0.1:7778" --geolocation 1 [--debug-relays] --log_level <debug|warn|...> --from <wallet> --chain-id <lava-chain> --strategy latency`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			usageServerAddr := viper.GetString(metrics.UsageServerFlagName)
	...
	cmdRPCConsumer.Flags().String(metrics.UsageServerFlagName, metrics.DisabledFlagOption, "the http address of the relay usage server api endpoint (example http://127.0.0.1:8080)")
