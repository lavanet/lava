		...
		go func(rpcEndpoint *lavasession.RPCEndpoint) error {
			...
			getOrCreateChainAssets := func() error {
				...
				if CollectOptimizerProviderDataFlag {
					consumerOptimizerDataCollector = metrics.NewConsumerOptimizerDataCollector(chainID, rpcEndpoint.ApiInterface, OptimizerProviderDataCollectionIntervalFlag, consumerMetricsManager)
				}

				newOptimizer := provideroptimizer.NewProviderOptimizer(options.strategy, averageBlockTime, baseLatency, options.maxConcurrentProviders, consumerOptimizerQoSClient, chainID, consumerOptimizerDataCollector)
