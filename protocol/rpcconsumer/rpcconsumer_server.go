...
func (rpccs *RPCConsumerServer) ServeRPCRequests(ctx context.Context, listenEndpoint *lavasession.RPCEndpoint,
	...
	rpccs.latestBlockEstimator = relaycore.NewLatestBlockEstimator()
	rpccs.chainListener, err = chainlib.NewChainListener(ctx, listenEndpoint, rpccs, rpccs, rpcConsumerLogs, chainParser, nil, consumerWsSubscriptionManager)
	if err != nil {
		return err
	}
		...
		if err != nil {
			...
			if err != nil {
				...
				if err == nil && relayResult != nil && relayResult.Reply != nil {
					...
					// Update our simple block height tracker for archive extension detection
					if relayResult.Reply.LatestBlock > 0 {
						rpccs.updateLatestBlockHeight(uint64(relayResult.Reply.LatestBlock), relayResult.ProviderInfo.ProviderAddress)
					}
...
func (rpccs *RPCConsumerServer) sendCraftedRelays(retries int, initialRelays bool) (bool, error) {
	...
	ok, relay, chainMessage, _ := rpccs.craftRelay(ctx)
	if !ok {
		return true, nil
	}
...
func (rpccs *RPCConsumerServer) getLatestBlock() uint64 {
	...
	if latest := rpccs.latestBlockHeight.Load(); latest > 0 {
		return latest
	}
...
func (rpccs *RPCConsumerServer) updateLatestBlockHeight(blockHeight uint64, providerAddress string) {
	for {
		...
		if blockHeight <= current {
			break // Only update if new block is higher
		}
		if rpccs.latestBlockHeight.CompareAndSwap(current, blockHeight) {
			break
		}
	}

	if providerAddress != "" && rpccs.latestBlockEstimator != nil {
		rpccs.latestBlockEstimator.Record(providerAddress, int64(blockHeight))
	}
}
...
func (rpccs *RPCConsumerServer) SendParsedRelay(
	...
	rpccs.appendHeadersToRelayResult(ctx, returnedResult, relayProcessor.ProtocolErrors(), relayProcessor, protocolMessage, protocolMessage.GetApi().GetName(), analytics)
	if err != nil {
	...
	if analytics != nil {
		...
		analytics.IsArchive = chainlib.IsArchiveRequest(protocolMessage)
		analytics.IsDebugTrace = chainlib.IsDebugOrTraceRequest(protocolMessage)
		analytics.IsBatch = chainlib.IsBatchRequest(protocolMessage)
...
// deepCopyRelayPrivateData creates a complete independent copy of RelayPrivateData
// to prevent race conditions when the same protocolMessage is accessed by multiple goroutines
...
func (rpccs *RPCConsumerServer) sendRelayToProvider(
				...
				if err != nil {
					...
					if cacheError == nil && reply != nil {
						...
						if reply.LatestBlock > 0 {
							rpccs.updateLatestBlockHeight(uint64(reply.LatestBlock), "")
						}
	...
	reqBlock = rpccs.resolveRequestedBlock(reqBlock, localRelayData.SeenBlock, latestBlockHashRequested, protocolMessage)
	...
	for providerPublicAddress, sessionInfo := range sessions {
		...
		go func(providerPublicAddress string, sessionInfo *lavasession.SessionInfo, relayData *pairingtypes.RelayPrivateData) {
			...
			// Update our simple block height tracker for archive extension detection
			if latestBlock > 0 {
				rpccs.updateLatestBlockHeight(uint64(latestBlock), providerPublicAddress)
			}
...
func (rpccs *RPCConsumerServer) relaySubscriptionInner(ctx context.Context, hashedParams string, endpointClient pairingtypes.RelayerClient, singleConsumerSession *lavasession.SingleConsumerSession, relayResult *common.RelayResult) (err error) {
	...
	if latestBlock > 0 {
		...
		rpccs.updateLatestBlockHeight(uint64(latestBlock), providerAddress)
	}
...
func (rpccs *RPCConsumerServer) appendHeadersToRelayResult(ctx context.Context, relayResult *common.RelayResult, protocolErrors uint64, relayProcessor RelayProcessorForHeaders, protocolMessage chainlib.ProtocolMessage, apiName string, analytics *metrics.RelayMetrics) {
