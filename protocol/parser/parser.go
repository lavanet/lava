...
func ParseByArg(rpcInput RPCInput, input []string, dataSource int) ([]interface{}, error) {
	...
	if len(input) != 1 {
		return nil, utils.LavaFormatError("invalid input format, input length", nil, &map[string]string{"input_len": strconv.Itoa(len(input))})
	}
	inp := input[0]
	param_index, err := strconv.ParseUint(inp, 10, 32)
	if err != nil {
		return nil, utils.LavaFormatError("invalid input format, input isn't an unsigned index", err, &map[string]string{"input": inp})
	}
	...
	default:
		...
		return nil, utils.LavaFormatError("Parse type unsupported in parse by arg, only list parameters are currently supported", nil, &map[string]string{"request": fmt.Sprintf("%s", unmarshaledDataTyped)})
	}
}
