...
func TestHappyFlowE2EEmergency(t *testing.T) {
	...
	for i := 0; i < len(consumerVirtualEpochs); i++ {
		css, err := csm.GetSessions(ctx, maxCuForVirtualEpoch, emptyRelayProcessor(), servicedBlockNumber, "", nil, common.NOSTATE, consumerVirtualEpochs[i]) // get a session
...
func TestHappyFlowEmergencyInConsumer(t *testing.T) {
	...
	css, err := csm.GetSessions(ctx, maxCuForVirtualEpoch, emptyRelayProcessor(), servicedBlockNumber, "", nil, common.NOSTATE, virtualEpoch) // get a session
	require.NoError(t, err)
