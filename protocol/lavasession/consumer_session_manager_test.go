...
func TestHappyFlow(t *testing.T) {
	...
	for _, cs := range css {
		...
		err = csm.OnSessionDone(cs.Session, servicedBlockNumber, cuForFirstRequest, time.Millisecond, cs.Session.CalculateExpectedLatency(2*time.Millisecond), (servicedBlockNumber - 1), numberOfProviders, numberOfProviders, false, false, nil)
		require.NoError(t, err)
...
func TestHappyFlowVirtualEpoch(t *testing.T) {
	...
	for _, cs := range css {
		...
		err = csm.OnSessionDone(cs.Session, servicedBlockNumber, maxCuForVirtualEpoch*(virtualEpoch+1), time.Millisecond, cs.Session.CalculateExpectedLatency(2*time.Millisecond), (servicedBlockNumber - 1), numberOfProviders, numberOfProviders, false, false, nil)
		require.NoError(t, err)
...
func TestSuccessAndFailureOfSessionWithUpdatePairingsInTheMiddle(t *testing.T) {
	...
	for j := 0; j < numberOfAllowedSessionsPerConsumer/2; j++ {
		...
		if rand.Intn(2) > 0 {
			err = csm.OnSessionDone(cs, servicedBlockNumber, cuForFirstRequest, time.Millisecond, cs.CalculateExpectedLatency(2*time.Millisecond), (servicedBlockNumber - 1), numberOfProviders, numberOfProviders, false, false, nil)
			require.NoError(t, err)
