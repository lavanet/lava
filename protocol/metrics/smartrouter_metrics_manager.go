...
type SmartRouterMetricsManager struct {
	...
	endpointInfo               *MappedLabelsGaugeVec   // lava_rpc_endpoint_info (+provider_name label)
...
func NewSmartRouterMetricsManager(options SmartRouterMetricsManagerOptions) *SmartRouterMetricsManager {
	...
	// Info metric — endpoint_id is the URL; provider_name is the human-readable label for dashboard joins
	endpointInfo := NewMappedLabelsGaugeVec(MappedLabelsMetricOpts{
		...
		Labels: []string{"spec", "apiInterface", "endpoint_id", "provider_name"},
	})
...
// endpoint_id is the URL; providerName is the human-readable label for dashboard joins.
func (m *SmartRouterMetricsManager) SetEndpointInfo(spec, apiInterface, endpointID, providerName string) {
	if m == nil {
		return
	}
	labels := map[string]string{"spec": spec, "apiInterface": apiInterface, "endpoint_id": endpointID, "provider_name": providerName}
...
// endpointID must be the URL (consistent with all relay/health/block metrics).
// providerName is the human-readable label stored in the info metric for dashboard joins.
...
// RecordRelaySuccess records a successful relay, updating both router-level and
// endpoint-level counters and histograms with the function label.
...
func (m *SmartRouterMetricsManager) SetRelayNodeErrorMetric(string, string) {}

func (m *SmartRouterMetricsManager) SetNodeErrorRecoveredSuccessfullyMetric(string, string, string) {
}
