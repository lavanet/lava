...
func (crsm *SmartRouterRelayStateMachine) Initialized() bool {
	return crsm.relayRetriesManager != nil && crsm.resultsChecker != nil
}

func (crsm *SmartRouterRelayStateMachine) SetRelayRetriesManager(relayRetriesManager *lavaprotocol.RelayRetriesManager) {
	crsm.relayRetriesManager = relayRetriesManager
}

func (crsm *SmartRouterRelayStateMachine) SetResultsChecker(resultsChecker ResultsCheckerInf) {
	crsm.resultsChecker = resultsChecker
}

func (crsm *SmartRouterRelayStateMachine) GetUsedProviders() *lavasession.UsedProviders {
	return crsm.usedProviders
}

func (crsm *SmartRouterRelayStateMachine) GetSelection() Selection {
	return crsm.selection
}

func (crsm *SmartRouterRelayStateMachine) appendRelayState(nextState *RelayState) {
	crsm.relayStateLock.Lock()
	defer crsm.relayStateLock.Unlock()
	crsm.relayState = append(crsm.relayState, nextState)
}

func (crsm *SmartRouterRelayStateMachine) getLatestState() *RelayState {
...
func (crsm *SmartRouterRelayStateMachine) stateTransition(relayState *RelayState, numberOfNodeErrors uint64) {
...
func (crsm *SmartRouterRelayStateMachine) shouldRetry(numberOfNodeErrors uint64) bool {
	batchNumber := crsm.usedProviders.BatchNumber()
...
func (crsm *SmartRouterRelayStateMachine) hasUnsupportedMethodErrorsInStateMachine() bool {
...
func (crsm *SmartRouterRelayStateMachine) retryCondition(numberOfRetriesLaunched int) bool {
...
func (crsm *SmartRouterRelayStateMachine) GetDebugState() bool {
	return crsm.debugRelays
}

func (crsm *SmartRouterRelayStateMachine) GetProtocolMessage() chainlib.ProtocolMessage {
...
func (crsm *SmartRouterRelayStateMachine) GetRelayTaskChannel() (chan RelayStateSendInstructions, error) {
	if !crsm.Initialized() {
...
func (crsm *SmartRouterRelayStateMachine) UpdateBatch(err error) {
