...
func (srsm *SmartRouterRelayStateMachine) retryCondition(numberOfRetriesLaunched int) bool {
	...
	if srsm.selection == relaycore.BestResult {
		...
		if srsm.resultsChecker.HasSuccessfulResults() {
			utils.LavaFormatTrace("[StateMachine] retryCondition: BestResult with success, no retry", utils.LogAttr("GUID", srsm.ctx))
			return false
		}
		// No successful results - check if a retry is warranted based on node errors
		hasRequiredResults, nodeErrors := srsm.resultsChecker.HasRequiredNodeResults(numberOfRetriesLaunched)
