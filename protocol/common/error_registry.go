...
// defaulting to ChainFamilyEVM for unknown chains.
...
func registerError(le *LavaError) *LavaError {
	...
	if _, exists := errorRegistryName[le.Name]; exists {
		panic(fmt.Sprintf("duplicate error name: %s (code %d)", le.Name, le.Code))
	}
