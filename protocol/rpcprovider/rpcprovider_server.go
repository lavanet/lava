...
func (rpcps *RPCProviderServer) ServeRPCRequests(
	...
	if rewardServer == nil {
		utils.LavaFormatWarning("disabled rewards for provider, reward server not defined", nil)
...
func (rpcps *RPCProviderServer) Relay(ctx context.Context, request *pairingtypes.RelayRequest) (reply *pairingtypes.RelayReply, err error) {
	...
	// the reply is compressed by gRPC itself when the consumer sent a compressed request,
	// see the enable-grpc-compression flag on the consumer side
...
func (rpcps *RPCProviderServer) finalizeSession(isRelayError bool, ctx context.Context, consumerAddress sdk.AccAddress, reply *pairingtypes.RelayReply, chainMessage chainlib.ChainMessage, relaySession *lavasession.SingleProviderSession, request *pairingtypes.RelayRequest, replyWrapper *chainlib.RelayReplyWrapper, err error) error {
	...
	if isRelayError {
		...
		// Unsupported-method errors are benign CU rollbacks — info-log only,
		// NO metric increment, NO "TryRelay Failed" tagging. Return early so
		// the failure path below is skipped entirely.
		...
		err = common.LogCodedWarning("TryRelay Failed", err, chainlib.ExtractLavaError(err), request.RelaySession.SpecId, 0, "",
			utils.Attribute{Key: "request.SessionId", Value: request.RelaySession.SessionId},
	...
	if relayError != nil {
		common.LogCodedError("OnSession Done failure", relayError, common.LavaErrorSessionAccountingFailed, request.RelaySession.SpecId, 0, "", utils.LogAttr("GUID", ctx))
	} else if sendRewards {
...
func (rpcps *RPCProviderServer) initRelay(ctx context.Context, request *pairingtypes.RelayRequest) (relaySession *lavasession.SingleProviderSession, consumerAddress sdk.AccAddress, chainMessage chainlib.ChainMessage, err error) {
	...
	if err != nil {
		...
		return nil, nil, nil, common.LogCodedWarning("Session Out of sync", lavasession.SessionOutOfSyncError, common.LavaErrorSessionOutOfSync, request.RelaySession.SpecId, 0, "", utils.Attribute{Key: "PrepareSessionForUsage_Error", Value: err.Error()}, utils.Attribute{Key: "GUID", Value: ctx}, utils.Attribute{Key: utils.KEY_REQUEST_ID, Value: ctx}, utils.Attribute{Key: utils.KEY_TASK_ID, Value: ctx}, utils.Attribute{Key: utils.KEY_TRANSACTION_ID, Value: ctx})
	}
...
func (rpcps *RPCProviderServer) ValidateRequest(chainMessage chainlib.ChainMessage, request *pairingtypes.RelayRequest, ctx context.Context) error {
	...
	if seenBlock < 0 {
		return common.LogCodedError("invalid seen block", nil, common.LavaErrorInvalidRelayRequest, request.RelaySession.SpecId, 0, "", utils.Attribute{Key: "GUID", Value: ctx}, utils.Attribute{Key: utils.KEY_REQUEST_ID, Value: ctx}, utils.Attribute{Key: utils.KEY_TASK_ID, Value: ctx}, utils.Attribute{Key: utils.KEY_TRANSACTION_ID, Value: ctx}, utils.Attribute{Key: "seenBlock", Value: seenBlock})
	}
	...
	if reqBlock != request.RelayData.RequestBlock {
		...
		if reqBlock != request.RelayData.RequestBlock {
			...
			common.LogCodedError("requested block mismatch between consumer and provider", nil, common.LavaErrorRequestBlockMismatch, request.RelaySession.SpecId, 0, "",
...
func (rpcps *RPCProviderServer) RelaySubscribe(request *pairingtypes.RelayRequest, srv pairingtypes.Relayer_RelaySubscribeServer) error {
	...
	if subscribed {
		...
		err = common.LogCodedWarning("failed subscribing", lavasession.SubscriptionInitiationError, common.LavaErrorSubscriptionInitFailed, request.RelaySession.SpecId, 0, "",
			utils.LogAttr("GUID", ctx),
...
func (rpcps *RPCProviderServer) TryRelaySubscribe(ctx context.Context, requestBlockHeight uint64, request *pairingtypes.RelayRequest, srv pairingtypes.Relayer_RelaySubscribeServer, chainMessage chainlib.ChainMessage, consumerAddress sdk.AccAddress, relaySession *lavasession.SingleProviderSession, relayNumber uint64) (subscribedSuccessfully bool, errRet error) {
	...
	go func() {
		...
		for {
			...
			case <-srv.Context().Done():
				...
				if err != nil {
					errRet = common.LogCodedError("Error RemoveConsumer", err, common.LavaErrorSubscriptionCleanupFailed, rpcps.rpcProviderEndpoint.ChainID, 0, "", utils.LogAttr("GUID", ctx))
				}
				return
			case subscribeReply, ok := <-subscribeRepliesChan:
				...
				if errRet != nil {
					...
					if strings.Contains(errRet.Error(), "Canceled desc = context canceled") {
						...
						errRet = common.LogCodedWarning("Got error from srv.Send()", errRet, common.ClassifyLegacyError(errRet, common.TransportGRPC), "", 0, "", utils.Attribute{Key: "GUID", Value: ctx}, utils.Attribute{Key: utils.KEY_REQUEST_ID, Value: ctx}, utils.Attribute{Key: utils.KEY_TASK_ID, Value: ctx}, utils.Attribute{Key: utils.KEY_TRANSACTION_ID, Value: ctx})
					}
	...
	if err != nil {
		...
		if relaySession != nil {
			...
			if relayError != nil {
				common.LogCodedError("Error OnSessionFailure", relayError, common.LavaErrorSessionAccountingFailed, rpcps.rpcProviderEndpoint.ChainID, 0, "")
			}
		}
	...
	if relaySession != nil {
		...
		if relayError != nil {
			common.LogCodedError("Error OnSessionDone", relayError, common.LavaErrorSessionAccountingFailed, rpcps.rpcProviderEndpoint.ChainID, 0, "")
		}
...
func (rpcps *RPCProviderServer) getSingleProviderSession(ctx context.Context, request *pairingtypes.RelaySession, consumerAddressString string) (*lavasession.SingleProviderSession, error) {
	...
	if err != nil {
		if lavasession.ConsumerNotRegisteredYet.Is(err) {
			...
			if !valid {
				return nil, common.LogCodedError("VerifyPairing, this consumer address is not valid with this provider", nil, common.LavaErrorInvalidRelayRequest, request.SpecId, 0, "",
					utils.Attribute{Key: "GUID", Value: ctx},
					utils.Attribute{Key: utils.KEY_REQUEST_ID, Value: ctx},
					utils.Attribute{Key: utils.KEY_TASK_ID, Value: ctx},
					utils.Attribute{Key: utils.KEY_TRANSACTION_ID, Value: ctx},
			...
			if getMaxCuError != nil {
				return nil, common.LogCodedError("ConsumerNotRegisteredYet: GetMaxCuForUser failed", getMaxCuError, common.LavaErrorConsumerNotRegistered, request.SpecId, 0, "",
					utils.Attribute{Key: "GUID", Value: ctx},
					utils.Attribute{Key: utils.KEY_REQUEST_ID, Value: ctx},
					utils.Attribute{Key: utils.KEY_TASK_ID, Value: ctx},
					utils.Attribute{Key: utils.KEY_TRANSACTION_ID, Value: ctx},
			...
			if err != nil {
				return nil, common.LogCodedError("Failed to RegisterProviderSessionWithConsumer", err, common.LavaErrorSessionNotFound, request.SpecId, 0, "",
					utils.Attribute{Key: "GUID", Value: ctx},
					utils.Attribute{Key: utils.KEY_REQUEST_ID, Value: ctx},
					utils.Attribute{Key: utils.KEY_TASK_ID, Value: ctx},
					utils.Attribute{Key: utils.KEY_TRANSACTION_ID, Value: ctx},
			...
			return nil, common.LogCodedError("Failed to get a provider session", err, common.LavaErrorSessionNotFound, request.SpecId, 0, "",
				utils.Attribute{Key: "GUID", Value: ctx},
				utils.Attribute{Key: utils.KEY_REQUEST_ID, Value: ctx},
				utils.Attribute{Key: utils.KEY_TASK_ID, Value: ctx},
				utils.Attribute{Key: utils.KEY_TRANSACTION_ID, Value: ctx},
...
func (rpcps *RPCProviderServer) TryRelayWithWrapper(ctx context.Context, request *pairingtypes.RelayRequest, consumerAddr sdk.AccAddress, chainMsg chainlib.ChainMessage) (reply *pairingtypes.RelayReply, replyWrapper *chainlib.RelayReplyWrapper, err error) {
	...
	// Get chain statistics needed for consistency checks and caching
...
func (rpcps *RPCProviderServer) tryGetRelayReplyFromCache(ctx context.Context, request *pairingtypes.RelayRequest, requestedBlockHash []byte, finalized bool) (reply *pairingtypes.RelayReply, ignoredMetadata []pairingtypes.Metadata, err error) {
	...
	if hashErr != nil {
		utils.LavaFormatWarning("TryRelay Failed computing hash for cache request", hashErr)
		return nil, nil, nil
	}
...
func (rpcps *RPCProviderServer) trySetRelayReplyInCache(ctx context.Context, request *pairingtypes.RelayRequest, chainMsg chainlib.ChainMessage, replyWrapper *chainlib.RelayReplyWrapper, latestBlock int64, averageBlockTime time.Duration, requestedBlockHash []byte, finalized bool, ignoredMetadata []pairingtypes.Metadata) {
	...
	if !isNodeError {
		...
		go func() {
			if hashErr != nil {
				utils.LavaFormatWarning("Failed hashing cache request", nil, utils.LogAttr("hashErr", hashErr))
				return
			}
...
func (rpcps *RPCProviderServer) GetParametersForCache(ctx context.Context, request *pairingtypes.RelayRequest, latestBlock int64, blockDistanceToFinalization uint32) (requestedBlockHash []byte, finalized bool) {
	...
	// Calculate if block is finalized using the consistent latest block
...
// Returns the total time slept before the condition was met or context was cancelled
...
// GetLatestBlockData fetches the latest block data including finalization information
