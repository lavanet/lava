...
func TestResourceLimiter_SelectBucket_CUPriority(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "Batch method with ampersands uses heavy bucket",
			computeUnits:   50,
			methodName:     "eth_getBlockByNumber&eth_getBlockByNumber&eth_getBlockByNumber",
			expectedBucket: BucketHeavy,
			reason:         "Batch methods are classified as heavy",
		},
		{
			name:           "Batch method with high CU uses heavy bucket",
			computeUnits:   500,
			methodName:     "debug_trace&debug_trace",
			expectedBucket: BucketHeavy,
			reason:         "Batch methods are classified as heavy regardless of CU",
		},
		{
			name:           "Single ampersand in method name treated as batch",
			computeUnits:   200,
			methodName:     "eth_call&eth_call",
			expectedBucket: BucketHeavy,
			reason:         "Any ampersand in method marks it as batch/heavy",
		},
	}
