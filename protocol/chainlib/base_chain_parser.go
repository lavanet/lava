...
func (bcp *BaseChainParser) UpdateBlockTime(newBlockTime time.Duration) {
	...
	utils.LavaFormatInfo("chainParser updated block time", utils.Attribute{Key: "newTime", Value: newBlockTime}, utils.Attribute{Key: "oldTime", Value: time.Duration(bcp.spec.AverageBlockTime) * time.Millisecond})
