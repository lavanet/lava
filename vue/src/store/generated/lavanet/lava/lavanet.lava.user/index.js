    ...
    actions: {
        ...
        async QueryUserStake({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryUserStake', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryUserStakeAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryUserStakeAll({ ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'UserStakeAll', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryUserStakeAll', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryUnstakingUsersAllSpecs({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryUnstakingUsersAllSpecs', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryUnstakingUsersAllSpecsAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryUnstakingUsersAllSpecsAll({ ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'UnstakingUsersAllSpecsAll', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryUnstakingUsersAllSpecsAll', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryStakedUsers({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                const key = params ?? {};
                const queryClient = await initQueryClient(rootGetters);
                let value = (await queryClient.queryStakedUsers(key.specName, key.output)).data;
                commit('QUERY', { query: 'StakedUsers', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryStakedUsers', payload: { options: { all }, params: { ...key }, query } });
