...
func GetChainProxy(nodeUrl string, nConns uint, sentry *sentry.Sentry, pLogs *PortalLogs) (ChainProxy, error) {
	...
	case "jsonrpc":
		return NewJrpcChainProxy(nodeUrl, nConns, sentry, pLogs), nil
	case "tendermintrpc":
		return NewtendermintRpcChainProxy(nodeUrl, nConns, sentry, pLogs), nil
	case "rest":
		return NewRestChainProxy(nodeUrl, sentry, pLogs), nil
	}
