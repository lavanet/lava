...
func (cp *tendermintRpcChainProxy) PortalStart(ctx context.Context, privKey *btcec.PrivateKey, listenAddr string) {
		...
		for {
			...
			if replyServer != nil {
				...
				if len(reply.Data) > 100 {
					cp.portalLogs.LogRequestAndResponse("tendermint ws", false, "ws", c.LocalAddr().String(), string(msg), string(reply.Data[:100]), "", nil)
				} else {
	...
	app.Post("/:dappId/*", func(c *fiber.Ctx) error {
		...
		if len(reply.Data) > 100 {
			cp.portalLogs.LogRequestAndResponse("tendermint http in/out", false, "POST", c.Request().URI().String(), string(c.Body()), string(reply.Data[:100]), msgSeed, nil)
		} else {
	...
	app.Get("/:dappId/*", func(c *fiber.Ctx) error {
		...
		if len(reply.Data) > 100 {
			cp.portalLogs.LogRequestAndResponse("tendermint http in/out", false, "GET", c.Request().URI().String(), "", string(reply.Data[:100]), "", nil)
		} else {
