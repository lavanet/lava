...
func askForRewards(staleEpochHeight int64) {
	...
	for ; idx < RETRY_INCORRECT_SEQUENCE && !success; idx++ {
		...
		if hasSequenceError { // a retry
			...
			myWriter.Reset()
			utils.LavaFormatInfo("Retrying with parsed sequence number:", nil, &map[string]string{
	...
	if hasSequenceError {
		utils.LavaFormatInfo("Sequence number error handling: ", nil, &map[string]string{
