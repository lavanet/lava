	...
	if apiInterface == "rest" {
		...
		for httpMethod, api := range mostImportantApisToTest {
			for _, api_value := range api {
				for i := 0; i < 20; i++ {
					reply, _, err := chainproxy.SendRelay(ctx, chainProxy, privKey, api_value, "", httpMethod)
					if err != nil {
						log.Println(err)
		...
		for i := 0; i < 100; i++ {
			...
			reply, _, err = chainproxy.SendRelay(ctx, chainProxy, privKey, OSMOSIS_NUM_POOLS_URL_REST, OSMOSIS_NUM_POOLS_DATA_REST, http.MethodGet)
			if err != nil {
				log.Println("1:" + err.Error())
				errors = append(errors, fmt.Sprintf("%s", err))
			} else {
