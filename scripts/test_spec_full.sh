...
if [ "$dry" = false ]; then
    screen -d -m -S consumers bash -c "source ~/.bashrc; lavap rpcconsumer testutil/debugging/logs/consumer.yml $EXTRA_PORTAL_FLAGS --geolocation 1 --log_level debug --from user1 --chain-id lava --allow-insecure-provider-dialing --allow-providers-with-protected-ips --metrics-listen-address ":7779" 2>&1 | tee $LOGS_DIR/PORTAL.log"
