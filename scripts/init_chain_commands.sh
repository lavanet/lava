...
echo; echo "#### Buying subscription for user1 ####"
...
echo; echo "#### Buying subscription for user2 ####"
lavad tx subscription buy DefaultPlan $(lavad keys show user2 -a) --enable-auto-renewal -y --from user2 --gas-adjustment "1.5" --gas "auto" --gas-prices $GASPRICE
...
lavad tx pairing bulk-stake-provider $BASE_CHAINS $PROVIDERSTAKE "$PROVIDER1_LISTENER,1" 1 $(operator_address) -y --delegate-commission 50 --delegate-limit $PROVIDERSTAKE --from servicer1 --provider-moniker "servicer1" --gas-adjustment "1.5" --gas "auto" --gas-prices $GASPRICE
