#!/bin/bash
# init_lava_only_with_node_three_providers_shared_state.sh
#
# Variant of init_lava_only_with_node_three_providers.sh used to validate the
# seenBlock/latestSyncData unification (branch: unify-seenBlock-globalLatestBlock).
# Differences from the base script:
#   1. Two rpcconsumer processes (consumers_1 and consumers_2), both with
#      --shared-state pointing at the same cache-be (127.0.0.1:20100). Lets
#      us exercise the §11.3 chain-wide cache key and verify cross-consumer
#      sync via the cache backend (scenario E1 in unification-demo-scenarios.md).
#   2. --enable-periodic-probe-providers + --debug-probes +
#      --majority-baseline-bucket-time-window 2m on both consumers — drives
#      Phase 2b consensus cycles every ~5s so scenario D2 is observable in
#      seconds rather than waiting for an epoch.
#   3. Uses LAV1 spec (testnet-2/specs/lava.json) and local lavad node URLs
#      ($LAVA_REST etc.) instead of the unify-branch base script's external
#      lavanet.xyz URLs — local URLs are reliable; external URLs depend on
#      gateway availability.
#   4. Both consumers run as --from user1. Per-curl identity (dappId) is set
#      via the `dapp-id` HTTP header on the client, not via the staking key,
#      so one staked user covers the whole demo.
...
echo "  provider 1     : screen -r provider1       ($PROVIDER1_LISTENER, metrics :7766)"
echo "  provider 2     : screen -r provider2       ($PROVIDER2_LISTENER, metrics :7756)"
echo "  provider 3     : screen -r provider3       ($PROVIDER3_LISTENER, metrics :7746)"
...
echo ""
echo "Logs:"
echo "  $LOGS_DIR/{CACHE_CONSUMER,PROVIDER1,PROVIDER2,PROVIDER3,CONSUMERS_1,CONSUMERS_2}.log"
echo ""
...
echo ""
echo "See unification-demo-env-setup.md for per-scenario walkthroughs."
echo "=========================================================================="
