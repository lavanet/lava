...
wait_next_block
lavad tx pairing stake-provider "AXELAR" $PROVIDERSTAKE "$PROVIDER1_LISTENER,1" 1 $(operator_address) -y --from servicer1 --provider-moniker "dummyMoniker" --delegate-limit 1000ulava --gas-adjustment "1.5" --gas "auto" --gas-prices $GASPRICE

sleep_until_next_epoch

screen -d -m -S provider1 bash -c "source ~/.bashrc; lavap rpcprovider \
$PROVIDER1_LISTENER AXELAR rest '$AXELAR_REST' \
$PROVIDER1_LISTENER AXELAR tendermintrpc '$AXELAR_RPC,$AXELAR_RPC' \
...
127.0.0.1:3360 AXELAR rest 127.0.0.1:3361 AXELAR tendermintrpc 127.0.0.1:3362 AXELAR grpc \
