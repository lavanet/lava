...
$EXTRA_PORTAL_FLAGS --geolocation 1 --log_level debug --from user1 --chain-id lava --allow-insecure-provider-dialing --allow-providers-with-protected-ips --metrics-listen-address ":7779" --relay-server-address "http://0.0.0.0:4300" 2>&1 | tee $LOGS_DIR/CONSUMERS.log" && sleep 0.25
