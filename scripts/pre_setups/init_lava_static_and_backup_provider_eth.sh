...
screen -d -m -S provider4 bash -c "source ~/.bashrc; lavap rpcprovider config/provider_examples/provider_example_cookbook/eth_provider_with_archive_debug.yml\
...
screen -d -m -S provider3 bash -c "source ~/.bashrc; lavap rpcprovider config/provider_examples/provider_example_cookbook/eth_provider_with_archive_debug1.yml\
...
screen -d -m -S consumers bash -c "source ~/.bashrc; lavap rpcsmartrouter config/consumer_examples/lava_consumer_static_with_backup_eth.yml \
$EXTRA_PORTAL_FLAGS --geolocation 1 --log_level trace --allow-insecure-provider-dialing --metrics-listen-address ":7779" --enable-provider-optimizer-auto-adjustment-of-tiers 2>&1 | tee $LOGS_DIR/CONSUMERS.log" && sleep 0.25
